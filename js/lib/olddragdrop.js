...
var webkit_tools = 
{
	...
	$:function(e)
	{
		if(typeof(e) == 'string')
		{
			...
			if (r) return r;
			else alert('element with id = ' + e + ' could not be found.');
		}
		return e;
	},
...
var webkit_draggable = function(r, ip)
{
	...
	{	    
		...
		var default_props = {scroll : false, revert : false, handle : this.root, zIndex : 1000, onStart : webkit_tools.empty, onEnd : webkit_tools.empty, direction: 'horizontal'};		
	...
	this.touchStart = function(event)
	{
        ...
        // if (p.direction == 'both' || p.direction == 'vertical')
        // {
        //     rs.top = this.root.style.top || '0px';
        //     rs.bottom = null;
        // }
        // 
        // if (p.direction == 'both' || p.direction == 'horizontal')
        // {
	...
	this.touchMove = function(event)
	{
		...
		if (p.direction == 'both' || p.direction == 'horizontal') rs.left = p.rx + delX + 'px';
		if (p.direction == 'both' || p.direction == 'vertical') rs.top  = p.ry + delY + 'px';
		
		if (p.direction == 'vertical')
		$('.debug').text('rs:'+rs.top+' ry:'+p.ry+' ty:'+p.ty+' delY:'+delY+' curY:'+curY);
		else
		$('.debug').text('rs:'+rs.left+' rx:'+p.rx+' tx:'+p.tx+' delX:'+delX+' curX:'+curX);
	...
	this.touchEnd = function(event)
	{
		...
		if(((p.revert) && (!dropped)) || (p.revert === 'always'))
		{
			//revert root
			var rs = r.style;
			if (p.direction == 'both' || p.direction == 'vertical') rs.top = (p.ry + 'px');
			if (p.direction == 'both' || p.direction == 'horizontal') rs.left = (p.rx + 'px');
		}
